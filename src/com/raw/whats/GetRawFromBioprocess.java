 ...
 * @Project 光合作用自动出题
 * @Comments 获得所有生物过程的原料，将生物过程、原料写入的txt文本中，以便出题使用
 ...
 * @author User
 */
public class GetRawFromBioprocess extends SPARQLParse{
	...
	private static final String strSubFilePath = "./src/data/raw/";
	private static final String strRawFileName = "raw.txt";
	
	//前缀
	private static final String strPrefix="http://www.co-ode.org/ontologies/ont.owl#";
	
	protected GetRawFromBioprocess() {}
	
	protected GetRawFromBioprocess(String strFile) {
		super(strFile);
...
//		System.out.println();
	...
	protected Struct recursive(String strBioprocess)
	{
		...
		String strFile=strSubFilePath+strRawFileName;
...
//			writeToTXT((String)s.struct_arr_raw.get(m)+",");
	...
	public static void main(String args[]){
		...
		OperateFile opfl = new OperateFile();
		GetRawFromBioprocess t1 = new GetRawFromBioprocess(strFilePath+strFileName);
		...
		System.out.println();
		opfl.readToTXT(strSubFilePath+strRawFileName);
