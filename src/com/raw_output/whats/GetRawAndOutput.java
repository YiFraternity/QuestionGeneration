 ...
 * @Project 光合作用自动出题
 * @Comments 获得所有生物过程的原料和产物，将生物过程、原料和产物写入的txt文本中，以便出题使用
 * @Noticed  子过程的原料和产物同时也是父过程的原料和产物
 *              eg.水是水的光解的原料，水也是光反应的原料，同时水也是光合作用的原料
 *           但如果生物过程的两个子过程中，一个子过程产物作为另一个子过程的原料，那么则不是生物过程的原料和产物
 ...
 * @author User
 */
public class GetRawAndOutput extends SPARQLParse{
	...
	protected GetRawAndOutput() {}
	
	//Constructor
	protected GetRawAndOutput(String strFile){
		super(strFile);
 	 ...
 	 * @param arr_raw
 	 */
 	protected void printRaw(String strBioprocess,ArrayList <String>arr_raw){
 		String strBio="<"+strPrefix+strBioprocess+">";
...
//		System.out.print("产物:");
		for(int i=0;i<arrlist.size();i++){
			String list_output=(String)arrlist.get(i);
...
//		System.out.println();
	...
	protected Struct recursive(String strBioprocess)
	{
		...
		//strFile为./src/data/raw_output/raw_output.xml文件
		String strFile=strSubFilePath+strRawOutputFileName;
		...
		/****写<原料>进入文件*/
//		opfl.writeToTXT("原料:",strFile);
		
		//将原料整理成字符串
		String s_arr_raw="";
		for(int m=0;m<s.struct_arr_raw.size();m++){
//			System.out.print((String)s.struct_arr_raw.get(m)+" ");
			s_arr_raw=s_arr_raw+(String)s.struct_arr_raw.get(m)+",";
//			writeToTXT((String)s.struct_arr_raw.get(m)+",");
		}
		s_arr_raw = deleteComma(s_arr_raw);
		
		/****写原料进入文件****/
		opfl.writeToTXT(s_arr_raw,strFile);
		opfl.writeToTXT("\n", strFile);
		/****写<产物>进入文件**/
...
//			writeToTXT((String)s.struct_arr_output.get(m)+",");
	...
	public static void main(String args[]){
		...
		OperateFile opfl = new OperateFile();
		GetRawAndOutput t1 = new GetRawAndOutput(strFilePath+strFileName);
...
//		filename=sc.nextLine();
		...
		System.out.println();
		opfl.readToTXT(strSubFilePath+strRawOutputFileName);
//		opfl.writeToCSV(strFilePath+raw_output_csv, opfl.readToTXT(strFilePath+strRawOutputFileName));
		opfl=null;
		t1=null;
		System.gc();
	}
